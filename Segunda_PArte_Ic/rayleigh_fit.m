...
% -------------
figure
temp=get(gcf,'position'); temp(3)=200*nch; temp(4)=650;
set(gcf,'position',temp); % units in pixels!

for ch=1:nch
    subplot(1,nch,ch)

    plot(Pr2(1:maxbin,ch), alt(1:maxbin)*1e-3); 
    xlabel('Pr2','fontsize',[10])  
    ylabel('Height a.g.l / km','fontsize',12)
    title([ num2str(lambda(ch)*1e9)],'fontsize',14)
    grid on
    hold on
    plot(Pr2_mol(1:maxbin,ch), alt(1:maxbin)*1e-3,'g','LineWidth',2); 
    plot(Pr2(RefBin(1),ch), alt(RefBin(1))*1e-3,'r*');
    plot(Pr2(RefBinTop(1),ch), alt(RefBinTop(1))*1e-3,'r*');
    hold off
...
for ch=1:nch
    ...
    plot(log10(Pr2(1:maxbin,ch)),alt(1:maxbin)*1e-3,'b');    
    xlabel('log10(Pr2)','fontsize',[10])  
    ylabel('Height a.g.l. / km','fontsize',12)
    title([ num2str(lambda(ch)*1e9)] ,'fontsize',14) 
    grid on 
    hold on
    plot(log10(Pr2_mol(1:maxbin,ch)),alt(1:maxbin)*1e-3,'g','LineWidth',2);   
    plot(log10(Pr2(RefBin(1),ch)), alt(RefBin(1))*1e-3,'r*');
    plot(log10(Pr2(RefBinTop(1),ch)), alt(RefBinTop(1))*1e-3,'r*');
    hold off
end
