...
% % % % % % % %     % for AN channel (head.ch(ch).photons==0), displace by dbin bins
% % % % % % % %     % for PC channel (head.ch(ch).photons==1), correct dead-time
% % % % % % % %     if (head.ch(ch).photons==0)
% % % % % % % %       % displace by dbin's
% % % % % % % %       tmpphy(1:nz-dbin) = tmpphy(1+dbin:nz);
% % % % % % % %       
...
% % % % % % % %     end
    ...
    if (dbin(ch)>0)
        % displace by dbin's
        tmpphy(1:nz-dbin(ch)) = tmpphy(1+dbin(ch):nz);
        
        % repeat the last dbin values to keep size of vectors
        tmpphy(nz-dbin(ch)+1:nz) = tmpphy(nz-dbin(ch)+1:nz);
        
    else

        % displace by dbin's
        tmpphy(1-dbin(ch):nz) = tmpphy(1:nz+dbin(ch));
